...
local function create_floating_file(location, opts)
  ...
  opts = vim.lsp.util.make_floating_popup_options(if_nil(width, 50), if_nil(height, 40), opts)
  ...
  local status_ok, _ = pcall(vim.api.nvim_set_option_value, "winbar", nil, { win = winnr })
  if not status_ok then
    return
  end
  vim.api.nvim_win_set_option(winnr, "winblend", 0)

  vim.api.nvim_win_set_cursor(winnr, { range.start.line + 2, range.start.character })
...
function M.Peek(what)
  ...
  if vim.tbl_contains(vim.api.nvim_list_wins(), M.floating_win) then
    ...
    local status_ok, _ = pcall(vim.api.nvim_set_option_value, "winbar", nil, { win = M.floating_win })
    if not status_ok then
      return
    end
    ...
    vim.api.nvim_buf_set_keymap(M.floating_buf, "n", "q", ":close<CR>", { noremap = true, silent = true })

    vim.api.nvim_buf_set_keymap(M.floating_buf, "n", "<ESC>", ":close<CR>", { noremap = true, silent = true })
  else
